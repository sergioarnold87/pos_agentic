...
from pykude.kude_nde.config import KudeNdeConfig
from pykude.xml_helpers import extract_de_data, parse_xml


class KudeNde(KudeBase):
    """
    Generate KuDE for Nota de Débito Electrónica (Electronic Debit Note).

    Usage:
        kude = KudeNde(xml=xml_content)
        kude.output("kude_nde.pdf")
    """

    def __init__(self, xml, config=None):
        super().__init__(orientation="P", unit="mm", format="A4")
        self.config = config or KudeNdeConfig()
